...
<%
	...
	rdate0						= Replace(SQLI(chkWord(upObj("rdate"))),"-","")
...
'	Response.Write "FID_NO : "& FID_NO &"<br>"
	...
	If flag = "W" Then							'쓰기
		...
		rso()
		SQL = " SELECT COUNT(*) FROM _orsvt010 WHERE rdate = '"& rdate0 &"' AND uno = "& FID_NO
'		Response.Write SQL &"<br>"
		rs.open SQL, dbcon
			ecnt			= CInt(rs(0))			'후기 등록일자에 해당 아이디 존재 여부
		rsc()
'		Response.Write "ecnt : "& ecnt &"<br>"
		...
		If ecnt <> 0 Then
			SQL = " INSERT INTO _opntt020 (uno, op, shipid, rsvid, point, note, ddate) VALUES (" _
				...
				& ",			'+'" _
				& ",			"& shipid _
				& ",			"& rsvInfo5(shipid,rdate0,FID_NO) _
				& ",			500" _
				& ",			'예약번호 "& rsvInfo5(shipid,rdate0,FID_NO) &"에 대한 후기글 등록-500포인트'" _
				& ",			getdate()" _
				& ")"
'			Response.Write SQL &"<br>"
'			dbcon.Execute SQL
			SQL = " INSERT INTO _opntt020 (uno, op, shipid, rsvid, point, note, ddate) VALUES (" _
				...
				& ",			"& shipid _
				& ",			"& rsvInfo5(shipid,rdate0,FID_NO) _
				& ",			"& 50 * upObj.Form("upFile").Count _
				& ",			'후기 글번호 "& mxseq &"에 대한 사진 "& upObj.Form("upFile").Count &"개 등록-"& 50 * upObj.Form("upFile").Count &"포인트'" _
				& ",			getdate()" _
				& ")"
'			Response.Write SQL &"<br>"
'			dbcon.Execute SQL
		Else
			OnlyAlert("글은 작성되었으나 해당 일자에 작성자 아이디로 출항 기록이 없으므로 포인트 적립은 불가능합니다.\n\n자세한 문의는 전화(041-674-1295) 바랍니다.")
		End If
		...
		For I = 1 To upObj.Form("upFile").Count
			...
			If upFile.Value <> "" Then
				...
				If upFile.ImageType > 0 Then					'이미지 파일
					...
					If blackFile(upFile.Value) = False Then
...
'						If ffnm <> "" Then				'tmp 삭제
'							If fs.FileExists(tmpPath & "\" & ffnm) Then
'								fs.DeleteFile tmpPath & "\" & ffnm, True
'							End If
'						End If
'						Set fs = Nothing
					Else
						upObj.Delete()
						Call OnlyAlert("정상적인 파일만 등록 가능합니다.")
					End If
