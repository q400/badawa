...
<%
	bbs_id						= 130								'10-공지/20-메모/50-요리/60-초보자/70-노하우/80-매니아/90-카풀/100-중고/110-레시피/120-잡다한소식/130-상품소개
		...
		<ul>
			...
			<li class="ib vt">
				<div id="mainwrap1">
					...
					<div class="mt10 mb10">
						<img src="/img/comu_shop_tle.gif" alt="상품소개" title="상품소개" />
					</div>
...
<form name="fm1" id="fm1" method="post">
					...
					<div class="mt20">
						...
						<div class="ib fright">
							<select name="cd3" style="width:70px;" class="bx1" onChange="location='?vw=<%=vw%>&cd3='+this.options[this.selectedIndex].value+''">
							<option value="" selected>선택</option>
							<option value="릴"<%If cd3 = "릴" Then%> selected<%End If%>>릴</option>
							<option value="낚시대"<%If cd3 = "낚시대" Then%> selected<%End If%>>낚시대</option>
							<option value="채비"<%If cd3 = "채비" Then%> selected<%End If%>>채비 하루</option>
							<option value="미끼"<%If cd3 = "미끼" Then%> selected<%End If%>>미끼</option>
							<option value="소품"<%If cd3 = "소품" Then%> selected<%End If%>>소품</option>
							<option value="기타"<%If cd3 = "기타" Then%> selected<%End If%>>기타</option>
							</select>
							<input type="text" name="cd2" class="bx1" style="width:100px; ime-mode:active;" />
...
%>
							<li style="width:160px;" class="vt mt20 ib">
								<div>
									<a href="shop_v.asp?seq=<%=rs("seq")%>&page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&vw=<%=vw%>"><img src="/data/shop/<%=fnm%>" width="160" class="gbox03" /></a>
								</div>
								<p>
									<a href="shop_v.asp?seq=<%=rs("seq")%>&page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&vw=<%=vw%>"><%=trimtext(rs("title"),20)%></a>
									<%If Date() - rs("ddate") < 2 Then%><img src="/img/icon/new03.gif" width="11" height="11" class="vm"><%End If%>
								</p>
								<p class="fc2">분류 : <%=rs("gubn")%></p>
								<p class="fc2">조회수 : <%=FormatNumber(rs("cnt"),0)%></p>
							</li>
<%
				rs.MoveNext
				i = i + 1
...
%>
					<div>
						<ul class="mt5 mb5">
							<li style="width:120px;" class="ib ct">분류</li>
							<li style="width:420px;" class="ib ct">제목</li>
...
%>
						<ul class="mt5 mb5">
							<li style="width:70px;" class="ib ct">
								<a href="shop_v.asp?seq=<%=rs("seq")%>&page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&vw=<%=vw%>"><img src="/data/shop/<%=fnm%>" width="60" /></a>
							</li>
							<li style="width:120px;" class="ib ct"><a href="shop_v.asp?seq=<%=rs("seq")%>&page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&vw=<%=vw%>"><%=rs("gubn")%></a></li>
							<li style="width:380px;" class="ml10 ib">
								<a href="shop_v.asp?seq=<%=rs("seq")%>&page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&vw=<%=vw%>"><%=trimtext(rs("title"),20)%></a>
								<%If Date() - rs("ddate") < 2 Then%><img src="/img/icon/new03.gif" width="11" height="11" class="vm"><%End If%>
							</li>
