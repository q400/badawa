...
'	Call checkLevel(FID_AUTH, 100, Request.ServerVariables("PATH_INFO"))
	...
	If seq <> "" Then
		rso()
		SQL = " SELECT	title, uno, nicknm, uip, cnt, ddate, contents FROM _obbst010 "& param &" AND seq = "& seq
		rs.open SQL, dbcon
		If Not rs.eof Then
			title				= rs("title")
			uno					= rs("uno")
		...
		rsc()
	Else
		Call noAlertGo("beginner.asp?page="& page &"&cd1="& cd1 &"&cd2="& cd2)
		Response.End
	End If
	...
	rso()					'이전글
	SQL = " SELECT TOP 1 seq, title FROM _obbst010 "& param &" AND seq < "& seq &" ORDER BY seq DESC "
	rs.open SQL, dbcon
	If Not rs.eof Then
		pvP = rs(0)
		pvTitle = rs(1)
	...
	rso()					'다음글
	SQL = "	SELECT TOP 1 seq, title FROM _obbst010 "& param &" AND seq > "& seq &" ORDER BY seq "
	rs.open SQL, dbcon
	If Not rs.eof Then
		ntP = rs(0)
		ntTitle = rs(1)
...
<%	If FID_ID = "" Then %>
	alert("로그인이 필요합니다.");
	this.document.location = "/mem/login.asp?preURL=/bbs1/beginner_v.asp?seq=<%=seq%>&page=<%=page%>";
	return;
<%	End If %>
	if($.trim($("#comment").val()).length == ""){
		alert("덧글내용을 입력하세요.");
		$("#comment").focus();
	}else if($.trim($("#comment").val()).length < 5){
		alert("최소 5자 이상은 입력하세요.");
		$("#comment").focus();
	}else if($.trim($("#comment").val()).length > 1000){
...
<script type="text/javascript">
	$(window).load(function(){ //$(window).load() must be used instead of $(document).ready() because of Webkit compatibility
		...
		$(".photosgallery-captions").sliderkit({
			...
			shownavitems:7,
			auto:true,
			panelfxspeed:150,				//하단 text 뿌려지는 속도
			delaycaptions:true
		});
		...
		<ul>
			...
			<li class="ib vt">
				<div id="mainwrap4">
					...
					<div class="mt10 mb10">
						<img src="/img/comu_beginner_tle.gif" alt="초보자교실" title="초보자교실" />
					</div>
					<div class="pt20"></div>
...
%>
							...
							s1.addVariable("file","/data/beginner/<%=fnm%>");
...
%>
							<video id="videobcg" preload="auto" autoplay="true" loop="noloop" width="400" controls>
								<source src="/data/beginner/<%=fnm%>" type="video/mp4" />
							</video>
<%
				End If
		Else
...
<%		if(FID_ID <> "" And CInt(FID_NO) = CInt(uno)) Or FID_AUTH = 10 Then %>
						<a href="beginner_w.asp?seq=<%=seq%>&page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&flag=M" class="btn btn25"><span>수정</span></a>
						<a href="javascript:goDelete();" class="btn btn25"><span>삭제</span></a>
<%		End If %>
						<a href="beginner.asp?page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&vw=<%=vw%>" class="btn btn25"><span>목록</span></a>
					</div>
