...
'	Call checkLevel(FID_AUTH, 100, Request.ServerVariables("PATH_INFO"))
	...
	If seq <> "" Then
		...
		rsc()
	Else
		Call noAlertGo("shop.asp?page="& page &"&cd1="& cd1 &"&cd2="& cd2)
		Response.End
	End If
...
<%	If FID_ID = "" Then %>
	alert("로그인이 필요합니다.         ");
	this.document.location = "/mem/login.asp?preURL=/bbs1/shop_v.asp?seq=<%=seq%>&page=<%=page%>";
	return;
<%	End If %>
	if (f.comment.value == "") {
		alert("덧글 내용을 입력하세요.       ");
...
<%		If (FID_ID <> "" And CInt(FID_NO) = CInt(uno)) Or FID_AUTH = 10 Then %>
						<a href="shop_w.asp?seq=<%=seq%>&page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&flag=M" class="btn btn25"><span>수정</span></a>
						<a href="javascript:goDelete();" class="btn btn25"><span>삭제</span></a>
<%		End If %>
						<a href="shop.asp?page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>&vw=<%=vw%>" class="btn btn25"><span>목록</span></a>
					</div>
