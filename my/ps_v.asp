...
'	Response.Write "<font color=#ffffff>auth : "& FID_AUTH &"</font><br>"
	...
	If seq <> "" Then
		rso()
		SQL = " SELECT seq, shipid, title, uno, uip, cnt, rdate, ddate FROM _obbst050 WHERE seq = "& seq &" AND uno = "& FID_NO
		rs.open SQL, dbcon
		If Not rs.eof Then
			shipid				= rs("shipid")
			title				= rs("title")
	...
	rso()					'이전글
	SQL = " SELECT	COUNT(*) FROM _opntt010 WHERE seq = "& seq &" AND note LIKE '조황후기%' "
	rs.open SQL, dbcon
		pcnt = CInt(rs(0))
	rsc()
'	Response.Write "<font color=#ffffff>pcnt : "& pcnt &"</font><br>"
%>
...
function goDelete() {
	var f = document.fm1;
	if (confirm("삭제하시겠습니까? 포인트가 적립된 경우 포인트가 소멸됩니다.       ")) {
		f.flag.value = "D";
		f.action = "ps_x.asp";
		f.method = "post";
		f.target = "nullframe";
							...
							<tr>
								<td>
									<table width="800" border="0" cellspacing="0" cellpadding="0">
										<tr>
											<td width="600"><font class="f17 ff fb fc8 ls"><%=rdate%>&nbsp;&nbsp;<%=meminfo(uno,"unamee")%> 님의 조황후기입니다.</font></td>
											<td class="rg">
...
%>
							...
							<tr>
								<td style="padding:0 0 5px 20px;"><%=db2html(rs("comment"))%></td>
							</tr>
							<tr>
...
%>
							...
							<tr>
								<td class="rg">
<%		If FID_NO <> "" Then %>
									<a href="#" class="btn btn25"><span>앨범담기</span></a>
<%			If (CInt(FID_NO) = uno Or FID_AUTH = 1) And pcnt = 0 Then %>
