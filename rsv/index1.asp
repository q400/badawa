...
<form name="fm1" id="fm1" method="post"></form>
						...
						<div class="mb10">
							<img src="/img/reser_s_tle01.gif" width="93" height="15" alt="예약가능인원" />&nbsp;&nbsp;&nbsp;&nbsp;
...
<%
		...
		If Not (rs.eof And rs.bof) Then
			...
			Do Until rs.EOF
%>
								<li class="ct ib" style="width:100px; border:1px solid #ccc;">
									<div class="ct" style="background:#ddd;"><%=rs("shipnm")%></div>
<%				If sstat(rs("shipid"), yy & mm & dd) = 1 Then				'탐사/정비/마감 중 하나의 경우 %>
									<div class="ct ib"><%=sstat3(rs("shipid"), yy & mm & dd, 3)%></div>
<%
				Else
					'예약가능인원
					If rsvInfo(rs("shipid"),yy & mm & dd,"gubn") = "D" Then			'독배
%>
									<div class="ct fcr f11">독배</div>
<%
					Else
						mcnt = shipinfo(rs("shipid"),"capa") - guestCount(rs("shipid"),yy & mm & dd)
						If mcnt = 0 Then				'만원 => 마감처리
%>
									<div class="ct fcr f11">마감</div>
<%						Else %>
									<div class="ct ff f11 ls"><%=mcnt%> 명</div>
<%
						End If
					End If
				End If
...
%>
						...
						<div class="mt20">
							<img src="/img/reser_s_tle02.gif" alt="예약하기" title="예약하기" />&nbsp;&nbsp;&nbsp;&nbsp;
							<font class="fc5 f15 fb ls vt"><%=yy%>-<%=mm%>-<%=dd%></font>&nbsp;&nbsp;<b class="f15 fc5 vt">(<%=getMool1(yy,mm,dd)%>)</b>
						</div>
