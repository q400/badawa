	...
	<center>
		<div id="admwrap0">
			...
			<div class="ib vt" id="admwrap1">
				<div id="poptitle2">
					<p class="lf">
						<span class="tt2">예약 유입환경 통계 (예약취소 제외)</span>
						<span class="ib fright"><a href="rsv2.asp?oyear=<%=oyear%>">>> 예약취소 포함 보기</a></span>
					</p>
					<div class="lf">
						<ul>
...
'	Response.Write "intSumPc : "& intSumPc &"<br>"
'	Response.Write "intSumMobile : "& intSumMobile &"<br>"

						pc21 = intSumPc / nanumsu * 150
						pc22 = 100 - pc21

						mo21 = intSumMobile / nanumsu * 150
						mo22 = 100 - mo21
%>
							<tr>
								<td class="ct"><%=intLoop%>월</td>
								<td>
...
<%						Else %>
											<td style="width:<%=pc21%>%; background-color:#b54854; height:20px;" title="PC <%=FormatNumber(intSumPc, 0)%> 건"></td>
											<td style="width:<%=pc22%>%; background-color:#fff; height:20px;" title="PC <%=FormatNumber(intSumPc, 0)%> 건"></td>
<%						End If %>
										</tr>
									</table>
									<table width="100%" id="list3">
										<tr>
<%						If intSumMobile = 0 Then %>
											<td colspan=2 style="height:20px; background-color:#fff;"></td>
<%						Else %>
											<td style="width:<%=mo21%>%; background-color:#65a8b4; height:20px;" title="모바일 <%=FormatNumber(intSumMobile, 0)%> 건"></td>
											<td style="width:<%=mo22%>%; background-color:#fff; height:20px;" title="모바일 <%=FormatNumber(intSumMobile, 0)%> 건"></td>
<%						End If %>
										</tr>
									</table>
								</td>
								<td class="rg pr10"><%=FormatNumber(intSumMobile + intSumPc, 0)%> 건</td>
							</tr>
<%					Next %>
							<tr>
								<td colspan=3 class="rg pr10"><b>예약 합계 : <font color="#fa5555"><%=FormatNumber(intCntPc + intCntMobile,0)%></font> 건</b></td>
							</tr>
						</tbody>
					</table>
					<!-- 리스트 끝 -->
