...
<%
	...
	rso()
	SQL = " SELECT	COUNT(*) FROM _opopt010 "& param
	rs.open SQL, dbcon, 3
				...
				<div id="poptitle2">
					<p class="lf">
						...
						<span class="ib fright">
							...
							<option value="sdate"<%If cd1 = "sdate" Then%> selected<%End If%>>시작년월</option>
							</select>
					...
					<table id="list2" class="wrapSub">
						<colgroup>
							...
							<col /><!-- 제목 -->
							<col style="width:100px;" /><!-- 시작일자 -->
							<col style="width:100px;" /><!-- 종료일자 -->
						...
						<thead>
							<tr style="height:30px;">
								...
								<th class="bdr-dash-r ct">시작일자</th>
								<th class="bdr-dash-r ct">종료일자</th>
								<th class="ct">등록일자</th>
							</tr>
						</thead>
						<tbody>
<%
		rso()
		SQL = " SELECT TOP "& pgsize &" * FROM _opopt010 "& param _
			& " AND seq NOT IN (SELECT TOP "& ((page-1) * pgsize) &" seq FROM _opopt010 "& param _
...
%>
							<tr height=30>
								<td class="ct"><%=j%></td>
								<td>&nbsp;&nbsp;&nbsp;
									<a href="pop_w.asp?seq=<%=rs("seq")%>&<%=pageParam%>&page=<%=page%>"><%=rs("title")%></a>
								</td>
								<td class="ct"><%=rs("sdate")%></td>
								<td class="ct"><%=rs("edate")%></td>
...
%>
							<tr height="100">
								<td class="ct vm" colspan="10">등록된 팝업이 없습니다.</td>
							</tr>
<%
		End If
		rsc()
%>
				...
				<div id="btnarea1">
					<a href="pop_w.asp" class="btn btn25"><span>팝업등록</span></a>
				</div>
			</div>
			<!-- admwrap1 E -->
		</div>
