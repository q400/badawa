<!-- #include virtual = "/_adm/inc/header8.asp" -->
<%
	Call checkAdm(FID_AUTH, 10, Request.ServerVariables("PATH_INFO"))

	seq							= Request("seq")
	cd1							= SQLI(Request("cd1"))
	cd2							= SQLI(Request("cd2"))
	page						= Request("page")
	flag						= SQLI(Request("flag"))

	If cd1 = "" Then cd1 = "title"
	If page = "" Then page = 1
	If flag = "" Then flag = "W"

	If seq <> "" Then
		rso()
		SQL = " SELECT seq, sdate, edate, title, fnm, ttop, lleft, winw, winh, link, ddate, contents FROM _opopt010 WHERE seq = "& seq
		rs.open SQL, dbcon
		If Not rs.eof Then
			sdate				= rs("sdate")
			edate				= rs("edate")
			title				= rs("title")
			fnm					= rs("fnm")					'파일명
			ttop				= rs("ttop")				'top좌표
			lleft				= rs("lleft")				'left좌표
			winw				= rs("winw")				'창 너비(기본 400px)
			winh				= rs("winh")				'창 높이(기본 400px)
			link				= rs("link")
			ddate				= rs("ddate")
			contents			= rs("contents")
		End If
		rsc()
		flag = "M"
	End If
	If ttop = "" Then ttop = 100
	If lleft = "" Then lleft = 0
	If winw = "" Then winw = 400
	If winh = "" Then winh = 400
	If sdate = "" Then sdate = Date()

'	nmparam						= setp(month(now)) & setp(day(now)) & setp(hour(now)) & setp(minute(now)) & setp(second(now))
'	Response.Write "value : "& nmparam &"<br>"
%>

<script src="/lib/jquery-1.7.1.js"></script>
<script src="/lib/ui/jquery.ui.core.js"></script>
<script src="/lib/ui/jquery.ui.widget.js"></script>
<script src="/lib/ui/jquery.ui.datepicker.js"></script>
<script type="text/javascript" src="/lib/js/jquery-ui-1.8.20.custom.min.js"></script>
<!-- <link rel="stylesheet" href="/lib/css/demos.css"> -->
<script type="text/javascript">
<!--
function upload(){
//	document.all.upload.style.visibility = "visible";
	mpop7('/inc/loader.html','ev','center',50,50,0);
}
function goSave(){
	var f = document.fm1;
//	카테고리 선택
//	if(!(fm1.cate[0].checked) && !(fm1.cate[1].checked) && !(fm1.cate[2].checked) && !(fm1.cate[3].checked) && !(fm1.cate[4].checked)){
//		alert("분류를 선택해 주세요.");
//		return;
//	}
//	if(f.title.value == ""){
//		alert("[필수] 제목을 입력하세요.");
//		f.title.focus();
//		return;
//	}
	if(f.sdate.value == ""){
		alert("[필수] 시작일자를 선택하세요.");
		f.sdate.focus();
		return;
	}
	if(confirm("입력하시겠습니까?")){
		upload();
		f.action = "pop_x.asp";
		f.method = "post";
//		f.target = "nullframe";
		f.submit();
	}else{
		return;
	}
}
function goDelete(){
	var f = document.fm1;
	if(confirm("삭제하겠습니까?")){
		f.flag.value = "D";
		f.action = "pop_x.asp";
		f.method = "post";
//		f.target = "nullframe";
		f.submit();
	}
}
function delPhoto(xidx){
	var f = document.fm1;
	if(confirm("첨부파일을 삭제하겠습니까?")){
		f.idx.value = xidx;
		f.flag.value = "delpic";
		f.action = "pop_x.asp";
		f.method = "post";
//		f.target = "nullframe";
		f.submit();
	}
}
function make(){						// 첨부파일 layer
	var len = document.fm1.filecnt.options[document.fm1.filecnt.selectedIndex].value;
	txtbox = " ";
	for (i=0; i<len; i++){
//		txtbox = txtbox + "<input type='file' name='upFile' class='file' style='width:600px;'><br>";
		txtbox = txtbox + "<div class='pt5'>사진 : <input type='file' name='upFile' class='file' style='width:485px;'></div>";
		txtbox = txtbox + "<div class='pb5'>설명 : <input type='text' name='upText' style='width:480px;'></div>";
	}
	layer17.innerHTML = txtbox;
}
//function init(){
//	upload();
//}
//window.onload = init;
//-->
</script>
<script>
$(function(){
	$("#datepicker").datepicker();
//	$("#format").val("yy-mm-dd");
//	$("#format").change(function(){
//	$("#datepicker").datepicker("<%=rdate%>", "dateFormat", "yy-mm-dd");//$(this).val()
});
$(function(){
	$("#datepicker2").datepicker();
});
</script>
<script type="text/javascript">
$(function(){			// Dialog
	$('#dialog').dialog({
			autoOpen: false
		,	width: 700
		,	height: 800
//		,	buttons: {
//				"Ok": function(){
//					$(this).dialog("close");
//				}
//		,
//				"Cancel": function(){
//					$(this).dialog("close");
//				}
//			}
	});
	// Dialog Link
	$('#dialog_link').click(function(){
		$('#dialog').dialog('enable').dialog('open');
		return false;
	});
	$('#dialog').click(function(){
		$('#dialog').dialog('enable').dialog('close');
		return false;
	});
	//hover states on the static widgets
//	$('#dialog_link, ul#icons li').hover(
//		function(){ $(this).addClass('ui-state-hover'); },
//		function(){ $(this).removeClass('ui-state-hover'); }
//	);
});
</script>


<form name="fm1" id=fm1 method="post" enctype="multipart/form-data">
<input type="hidden" name="seq" id=seq value="<%=seq%>">
<input type="hidden" name="cd1" id=cd1 value="<%=cd1%>">
<input type="hidden" name="cd2" id=cd2 value="<%=cd2%>">
<input type="hidden" name="idx" id=idx>
<input type="hidden" name="page" id=page value="<%=page%>">
<input type="hidden" name="flag" id=flag value="<%=flag%>">
<div id="wrap">
	<div>
		<!-- #include virtual = "/_adm/inc/top.asp" -->
	</div>
	<center>
		<div id="admwrap0">
			<div class="ib vt" id="admLeft"><!-- #include virtual = "/_adm/inc/left.asp" --></div>
			<div class="ib vt" id="admwrap1">
				<div id="poptitle2">
					<p class="lf">
						<span class="tt2">팝업관리</span>
						<span class="ib fright"></span>
					</p>
					<table id="list2" class="wrapSub">
						<colgroup>
							<col style="width:120px;" />
							<col style="width:280px;" />
							<col style="width:120px;" />
							<col />
						</colgroup>
						<tbody>
							<tr height=30>
								<th class="ct">제목</th>
								<td colspan=3>
									<input type="text" name="title" maxlength="100" value="<%=title%>" style="width:600px; ime-mode:active;">
								</td>
							</tr>
							<tr>
								<th class="ct">내용</th>
								<td colspan=3>
									<textarea name="contents" id="contents" class="tarea" style="width:600px; height:245px; ime-mode:active;"><%=contents%></textarea>
								</td>
							</tr>
							<tr>
								<th class="ct">이미지등록</th>
								<td colspan=3>
									<div>
<%
	If fnm <> "" Then
%>
										<a href="#" id="dialog_link"><img src="/data/notice/<%=fnm%>" width="100" alt="<%=fnm%>"></a>
										<input type="checkbox" name="cbox" value="<%=seq%>">
										<div id="dialog" title="이미지 크게보기" style="display:none;">
											<p class="ct"><img src="/data/notice/<%=fnm%>" class="ct" style="cursor:pointer;"></p>
										</div>
<%
	End If
%>
									</div>
									<input type="file" name="upFile" class="file" style="width:600px;">
								</td>
							</tr>
							<tr>
								<th class="ct">기간</th>
								<td colspan=3>
									<div class="demo">
									시작일자 : <input type="text" name="sdate" class="ct" id="datepicker" value="<%=sdate%>" style="width:100px;">
									&nbsp;&nbsp;&nbsp;
									종료일자 : <input type="text" name="edate" class="ct" id="datepicker2" value="<%=edate%>" style="width:100px;">
									</div>
								</td>
							</tr>
							<tr>
								<th class="ct">팝업크기</th>
								<td colspan=3>
									너 &nbsp;비 (폭) : <input type="text" name="winw" value="<%=winw%>" style="width:50px;"> px
									&nbsp;&nbsp;&nbsp;
									높 &nbsp;이 : <input type="text" name="winh" value="<%=winh%>" style="width:50px;"> px
								</td>
							</tr>
							<tr>
								<th class="ct">팝업위치</th>
								<td colspan=3>
									"위"로부터 : <input type="text" name="ttop" value="<%=ttop%>" style="width:50px;"> px
									&nbsp;&nbsp;&nbsp;
									"좌"로부터 : <input type="text" name="lleft" value="<%=lleft%>" style="width:50px;"> px
								</td>
							</tr>
							<tr>
								<th class="ct">Link URL</th>
								<td colspan=3>
									<input type="text" name="link" maxlength="300" value="<%=link%>" style="width:600px; ime-mode:inactive;">
								</td>
							</tr>
							<tr>
								<td colspan=4 class="ct">
									<span class="fcr pl20">※ 팝업은 최대 3개까지 가능합니다.</span>
								</td>
							</tr>
						</tbody>
					</table>
					<!-- 리스트 끝 -->
					<div id="btnarea1">
<%	If flag = "W" Then %>
						<a href="javascript:goSave();" class="btn btn25"><span>저장</span></a>
<%	Else %>
						<a href="javascript:goSave();" class="btn btn25"><span>수정</span></a>
						<a href="javascript:goDelete();" class="btnr btn25"><span>삭제</span></a>
<%	End If %>
						<a href="pop.asp?page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>" class="btn btn25"><span>목록</span></a>
					</div>
				</div>
				<!-- poptitle2 E -->
			</div>
			<!-- admwrap1 E -->
		</div>
	</center>
	<div>
		<!-- #include virtual = "/_adm/inc/footer.asp" -->
	</div>
</div>
</form>
<%	Set cx = Nothing %>
