				...
				<div id="poptitle2">
					<p class="lf">
						...
						<span class="ib fright">
							...
							<a href="/_adm/mem/point02.asp" class="btn btn25"><span>새로고침</span></a>
						</span>
					</p>
					...
					<table id="list2" class="wrapSub">
						...
						<thead>
							<tr style="height:30px;">
								...
								<th class="bdr-dash-r ct">해당글번호</th>
...
<%
		...
		SQL = " SELECT TOP "& pgsize &" * FROM _opntt010 "& param _
			& " AND ddate NOT IN (SELECT TOP "& ((page-1) * pgsize) &" ddate FROM _opntt010 "& param _
...
%>
							<tr height=30>
								...
								<td class="ct"><a href="#" onClick="return mpop5('point_w.asp?idx=<%=rs("idx")%>&gubn=talk','ev','center',480,300,0);"><%=FormatNumber(rs("point"),0)%></a></td>
								<td class="ct"><a href="#" onClick="return mpop5('point_w.asp?idx=<%=rs("idx")%>&gubn=talk','ev','center',480,300,0);"><%=meminfo(rs("uno"),"uname")%></a></td>
								<td class="ct"><%=rs("seq")%></td>
								<td class="ct"><a href="#" onClick="return mpop5('point_w.asp?idx=<%=rs("idx")%>&gubn=talk','ev','center',480,300,0);"><%=Left(rs("ddate"),10)%></a></td>
								<td>&nbsp;&nbsp;&nbsp;<a href="#" onClick="return mpop5('point_w.asp?idx=<%=rs("idx")%>&gubn=talk','ev','center',480,300,0);"><%=rs("note")%></a></td>
