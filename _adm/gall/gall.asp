...
<input type="hidden" name="page" value="<%=page%>">
					...
					<p class="lf">
						...
						<span class="ib fright">
							...
							<a href="javascript:goSearch();" id=btnSearch class="btn btn25"><span>검색</span></a>
							<a href="gall.asp" id=first class="btn btn25"><span>첫화면</span></a>
...
%>
							<tr height="32">
								...
								<td align="center" bgcolor="f1f1f1" class="fc2">
									<a href="gall_w.asp?seq=<%=rs("seq")%>&sdate=<%=rs("wdate")%>&&page=<%=page%>&<%=pageParam%>"><%=shipInfo(rs("shipid"),"shipnm")%></a>
								</td>
								<td bgcolor="f1f1f1" class="fc2 lf">&nbsp;
									<a href="gall_w.asp?seq=<%=rs("seq")%>&sdate=<%=rs("wdate")%>&page=<%=page%>&<%=pageParam%>"><%=rs("title")%></a></td>
...
%>
				...
				<div id="btnarea1">
					<a href="gall_img.asp?shipid=<%=shipid%>" class="btn btn25"><span>사진등록</span></a>
				</div>
</form>
			</div>
		</div>
