...
<input type="hidden" name="page" value="<%=page%>">
					...
					<p class="lf">
						...
						<span class="ib fright">
							...
							<option value=""<%If shipid = "" Then%> selected<%End If%>>선박명[선박번호]</option>
<%
	rso()
...
<%
		rso()
		SQL = " SELECT TOP "& pgsize &" * FROM _obbst020 "& param _
			& " AND wdate NOT IN (SELECT TOP "& ((page-1) * pgsize) &" wdate FROM _obbst020 "& param _
		...
		If Not (rs.eof And rs.bof) Then
			...
			Do Until rs.EOF
				...
				SQL = " SELECT ISNULL(COUNT(*),0) FROM _ogalt010 WHERE seq = "& rs("seq")
				rs3.open SQL, dbcon
					pcnt = rs3(0)
				rs3.close
				Set rs3 = Nothing
%>
							<tr height="32">
								<td class="ct"><%=j%></td>
								<td class="ct">
									<a href="gallery_w.asp?seq=<%=rs("seq")%>&sdate=<%=rs("wdate")%>&cd1=<%=cd1%>&cd2=<%=cd2%>&page=<%=page%>&shipid=<%=shipid%>">[<%=rs("shipid")%>]<%=shipInfo(rs("shipid"),"shipnm")%></a>
								</td>
								<td>&nbsp;&nbsp;&nbsp;
									<a href="gallery_w.asp?seq=<%=rs("seq")%>&sdate=<%=rs("wdate")%>&cd1=<%=cd1%>&cd2=<%=cd2%>&page=<%=page%>&shipid=<%=shipid%>"><%=rs("title")%></a></td>
								<td class="ct">
<%
				If pcnt = 0 Then
%>
									<b class="fcr"><%=rs("wdate")%></b>
<%
				Else
%>
									<span class="fcr"><%=rs("wdate")%></span>
<%
				End If
%>
								</td>
...
<%		End If %>
				...
				<div id="btnarea1">
					<a href="gallery_w.asp?shipid=<%=shipid%>" class="btn btn25"><span>사진등록</span></a>
				</div>
</form>
			</div>
		</div>
