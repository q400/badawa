...
//		txtbox = txtbox + "<input type='file' name='upFile' style='width:600px;'><br>";
		txtbox = txtbox + "<div class='mt5 ml10'>사진 : <input type='file' name='upFile' class='file' style='width:485px;'></div>";
		txtbox = txtbox + "<div class='mb5 ml10'>설명 : <input type='text' name='upText' style='width:480px;'></div>";
	}
...
<input type="hidden" name="nicknm" value="제임스">
<input type="hidden" name="idx">

				<div id="poptitle2">
					<p class="lf">
						<span class="tt2">제임스 이야기 글<%If flag = "W" Then%>쓰기<%Else%>수정<%End If%></span>
					...
					<table id="list2" class="wrapSub">
						...
						<tbody>
							...
							<tr height=30>
								<th class="ct">사진등록</th>
								<td colspan=3>
									<table border=0 id="list3">
										<tr>
<%
	If seq <> "" Then
		rso()
		SQL = " SELECT idx, fpath, fnm, onm, fsz, fwd, ext, best FROM _obbst011 WHERE seq = "& seq &" AND ext NOT IN ('flv','wmv','movie') "
		rs.open SQL, dbcon
		If Not rs.eof Then
			k = 1
			While Not rs.eof
%>
											<td class="vt">
												...
												<a href="javascript:Popup('/inc/imgv.asp?seq=<%=seq%>&idx=<%=rs("idx")%>&op=news',820,770,100,50,1,1,1);">
												<img src="/data/news/<%=rs("fnm")%>" width="100" alt="<%=rs("onm")%>"></a>
...
%>
							...
							<tr height=30>
								...
								<td colspan=3>
									<div class="fc8 mt5">※ 유투브를 등록한 후 해당 link를 연동합니다.</div>
<%
	If seq <> "" Then
		rso()
		SQL = " SELECT TOP 1 idx, fpath, fnm, onm, fsz, fwd, ext, best FROM _obbst011 WHERE seq = "& seq &" AND ext IN ('flv','wmv','movie') "
		rs.open SQL, dbcon
		If Not rs.eof Then
			vodUrl = rs("fnm")
			rs.MoveNext
	...
	End If
%>
									<input type="text" name="vodUrl" id="vodUrl" maxlength="200" class="mt5 mb5" value="<%=vodUrl%>" style="width:600px;" />
								</td>
							</tr>
							<!--
							<tr height=30>
								<th class="ct">동영상등록</th>
								<td colspan=3 class="mt5 mb5">
<%
	If seq <> "" Then
		rso()
		...
		If Not rs.eof Then
			While Not rs.eof
%>
									<a href="/data/dld.asp?path=news&file=<%=rs("fnm")%>"><%=rs("onm")%></a>
									<input type="checkbox" name="cbox" value="<%=rs("idx")%>">
<%
				rs.MoveNext
			Wend
