...
<input type="hidden" name="idx">
				...
				<div id="poptitle2">
					<p class="lf">
						<span class="tt2">초보자교실 글<%If flag = "W" Then%>쓰기<%Else%>수정<%End If%></span>
					...
					<table id="list2" class="wrapSub">
						...
						<tbody>
							...
							<tr height=30>
								<th class="ct">분류선택</th>
								<td colspan=3>
									<select name="gubn" class="bx" style="width:120px;">
...
%>
											<td class="vt">
												...
												<a href="javascript:Popup('/inc/imgv.asp?seq=<%=seq%>&idx=<%=rs("idx")%>&op=beginner',820,770,100,50,1,1,1);">
												<img src="/data/beginner/<%=rs("fnm")%>" width="100" alt="<%=rs("onm")%>"></a>
...
<%
	If seq <> "" Then
		...
		If Not rs.eof Then
			While Not rs.eof
%>
									<a href="/data/dld.asp?path=beginner&file=<%=rs("fnm")%>"><%=rs("onm")%></a>
									<input type="checkbox" name="cbox" value="<%=rs("idx")%>">
<%
				rs.MoveNext
			Wend
...
<%			End If %>
						<a href="beginner.asp?page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>" class="btn btn25"><span>목록</span></a>
					</div>
				</div>
				<!-- poptitle2 E -->
			</div>
