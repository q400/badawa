	...
	<center>
		<div id="admwrap0">
			...
			<div class="ib vt" id="admwrap1">
				<div id="poptitle2">
					<p class="lf">
						<span class="tt2">상품소개 글<%If flag = "W" Then%>쓰기<%Else%>수정<%End If%></span>
					...
					<table id="list2" class="wrapSub">
						...
						<tbody>
							...
							<tr height=30>
								<th class="ct">분류선택</th>
								<td colspan=3>
									<select name="gubn" style="width:150px;">
...
%>
											<td class="vt">
												...
												<a href="javascript:Popup('/inc/imgv.asp?seq=<%=seq%>&idx=<%=rs("idx")%>&op=shop',820,770,100,50,1,1,1);">
												<img src="/data/shop/<%=rs("fnm")%>" width="100" alt="<%=rs("onm")%>"></a>
...
<%			End If %>
						<a href="shop.asp?page=<%=page%>&cd1=<%=cd1%>&cd2=<%=cd2%>" class="btn btn25"><span>목록</span></a>
					</div>
				</div>
				<!-- poptitle2 E -->
			</div>
